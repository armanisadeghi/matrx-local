...
| Where | What |
|---|---|
| `desktop/src/lib/org/active-org.ts` | THE store (2026-09-21): one state value (`getActiveOrganizationSnapshot` / `subscribeActiveOrganization`, React via `use-active-organization.ts`), one persistent value (`matrx-local.active-organization.v2`, per signed-in user under one key; the v1 value migrates once), one write (`setActiveOrganization`). Order is device selection → sole membership → hold. `requireActiveOrganizationId({ timeoutMs? })` IS the hold: it raises the picker, waits for the set, and resumes. Every set is pushed to the engine. |
| `desktop/src/features/org/OrganizationSwitcher.tsx` | THE selector (2026-09-21), mounted once in `QuickActionBar` (top bar). Shows the true value — the set organization's name, or "Choose organization" in attention colors — and changes the store from one click. The "Change organization" menu items in the sidebar and user menu were removed; this is the one control. |
| `desktop/src/features/org/OrganizationPickerDialog.tsx` | The same store/list in a modal, for a HELD request. Registers the `choose_organization` action-needed handler (the sidecar's route) and re-states this Mac's pick to the engine whenever it connects and whenever the account changes. When the engine publishes `organization_required` while this device already has an answer, the answer is re-sent and the person is NOT asked again. |
| `app/api/organization_routes.py` PUT + `app/services/coding_sessions/service.py` | THE ONE CHOICE also answers the server's coding-session filing question (2026-09-21): a SET is mirrored to aidream `/coding-sessions/connection/organization` (best-effort), and the publisher answers the server's `organization_required` hold from the device value before it ever registers the "Your Claude Code sessions are waiting for an organization" card. |
