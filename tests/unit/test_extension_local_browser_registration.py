...
def _fresh(*, revision: int = 4, owner: tuple[str, str] = ("user", "session")) -> FreshContext:
...
def _message(**changes: object) -> dict[str, object]:
    message: dict[str, object] = {
        "type": "local_browser.register", "version": 1,
        "engine_boot_id": BOOT, "expected_revision": 4,
        "extension_generation": GENERATION, "connection_id": CONNECTION,
    }
    message.update(changes)
    return message


def _wire(monkeypatch: pytest.MonkeyPatch, context: Context) -> tuple[manager.ExtensionSessionRegistry, str, Socket]:
    ...
    monkeypatch.setattr(routes, "register_local_browser_session", registry.register_local_browser)
...
@pytest.mark.anyio
async def test_registration_acknowledges_the_exact_validated_candidate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert socket.frames == [{
        "type": "local_browser.registration", "version": 1, "status": "acknowledged",
        "engine_boot_id": BOOT, "expected_revision": 4,
        "extension_generation": GENERATION, "connection_id": CONNECTION,
    }]
...
@pytest.mark.anyio
async def test_context_change_while_identity_awaits_refuses_the_submitted_candidate(monkeypatch: pytest.MonkeyPatch) -> None:
    registry, session_id, socket = _wire(monkeypatch, Context(_fresh(), _fresh(revision=5), _fresh(revision=5)))
    assert await routes._handle_extension_message(session_id, _message())
    assert socket.frames == [{
        "type": "local_browser.registration", "version": 1, "status": "refused",
        "engine_boot_id": BOOT, "expected_revision": 4,
        "extension_generation": GENERATION, "connection_id": CONNECTION,
...
@pytest.mark.anyio
async def test_old_socket_cannot_install_and_duplicate_candidate_is_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    registry, session_id, socket = _wire(monkeypatch, Context(*[_fresh() for _ in range(9)]))
