    ...
    func acquire(key: String, context: LAContext, completion: @escaping (Result<Grant, Error>) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            case let .success((session, state, refresh)):
                ...
                else { self.validate(token: session.access_token, subject: session.subject, generation: state.generation, key: key) { result in completion(result.map { Grant(accessToken: session.access_token, subject: $0.sub, generation: state.generation) }) } }
            }
        }
    }
...
extension CredentialProviderViewController {
    ...
    func beginPasswordRequest(_ serviceIdentifiers: [ASCredentialServiceIdentifier]) {
        if let prior = nativePasswordOperation, !prior.terminal { prior.terminal = true; prior.phase = .cancelled }
        let identifiers = serviceIdentifiers.compactMap { value -> (String, String)? in
            let type = value.type == .domain ? "domain" : (value.type == .URL ? "url" : "")
            guard !type.isEmpty, value.identifier.utf8.count <= 2048 else { return nil }; return (type, value.identifier)
        }
        let operation = NativePasswordOperation(identifiers); nativePasswordOperation = operation
    ...
    private func receivedOrganizations(_ result: Result<(Data, HTTPURLResponse), Error>, grant: NativeVaultSessionAccess.Grant, operation: NativePasswordOperation) {
        guard current(operation), operation.generation == grant.generation, operation.subject == grant.subject else { return }
        do {
            let (data, response) = try result.get(); guard response.statusCode == 200 else { throw EnrollmentError.message("Vault organizations are unavailable. Try again.") }
    ...
    private func receivedMatches(_ result: Result<(Data, HTTPURLResponse), Error>, organization: NativeOrganization, grant: NativeVaultSessionAccess.Grant, operation: NativePasswordOperation) {
        guard current(operation), operation.generation == grant.generation, operation.subject == grant.subject else { return }
        do {
            let (data, response) = try result.get(); guard response.statusCode == 200 else { throw EnrollmentError.message("No matching password is available.") }
            ...
            let alert = NSAlert(); alert.messageText = "Choose a saved password"; alert.informativeText = "Only matching passwords in \(organization.name) are shown."
        ...
        } catch { cancelPassword(operation, "No matching password is available.") }
    }
    ...
    private func receivedMaterial(_ result: Result<(Data, HTTPURLResponse), Error>, grant: NativeVaultSessionAccess.Grant, operation: NativePasswordOperation) {
        guard current(operation), operation.generation == grant.generation, operation.subject == grant.subject else { return }
        do { let (data, response) = try result.get(); guard response.statusCode == 200 else { throw EnrollmentError.message("The selected password is no longer available.") }; let credential = try NativePasswordCodec.materialized(data); operation.phase = .completed; operation.terminal = true; extensionContext.completeRequest(withSelectedCredential: ASPasswordCredential(user: credential.username, password: credential.password), completionHandler: { [weak self] _ in Task { @MainActor in self?.nativePasswordOperation = nil } }) } catch { cancelPassword(operation, "The selected password is no longer available.") }
    }
    private func current(_ operation: NativePasswordOperation) -> Bool { nativePasswordOperation === operation && !operation.terminal }
    private func cancelPassword(_ operation: NativePasswordOperation, _ message: String) { guard current(operation) else { return }; operation.terminal = true; operation.phase = .cancelled; nativePasswordOperation = nil; extensionContext.cancelRequest(withError: NSError(domain: ASExtensionErrorDomain, code: ASExtensionError.userCanceled.rawValue, userInfo: [NSLocalizedDescriptionKey: message])) }
}
