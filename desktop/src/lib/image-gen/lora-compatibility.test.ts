...
describe("LoRA family compatibility", () => {
  ...
  it("defaults to confirmed matches and reveals every family on override", () => {
    ...
    expect(loraVisibleForModel(lora("1", "unknown"), zImage, false)).toBe(false);
  ...
  it("preserves selections and disables only confirmed cross-family adapters", () => {
    ...
    const result = disableIncompatibleLoraSelections(
      selections,
      [lora("klein", "flux2"), lora("zimage", "z-image"), lora("unknown", "unknown")],
      zImage,
    );
  ...
  it("scales filtering and search across hundreds of entries", () => {
    ...
    expect(library.filter((entry) => loraVisibleForModel(entry, klein, false))).toHaveLength(250);
    expect(library.filter((entry) => loraMatchesSearch(entry, "style 42"))).toHaveLength(11);
  });
});
