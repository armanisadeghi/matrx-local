...
function parseOutput(
  result: unknown,
): { text?: string; image?: string; type?: string } | null {
  try {
    const d = result as { output?: string; type?: string };
    if (!d) return null;
    if (d.type === "error")
      return { text: d?.output ?? "Error", type: "error" };
    if (d.output) {
      try {
...
function stepToInvoke(step: AutoStep): {
  toolName: string;
  params: Record<string, unknown>;
} {
  switch (step.type) {
    case "navigate":
      return { toolName: "BrowserNavigate", params: { url: step.params.url } };
    case "click":
      return {
    ...
    case "type":
      return {
        ...
        params: { selector: step.params.selector, text: step.params.text },
      };
    case "extract":
      return {
        toolName: "BrowserExtract",
        params: step.params.selector ? { selector: step.params.selector } : {},
      };
    case "screenshot":
      return { toolName: "BrowserScreenshot", params: {} };
    case "eval":
      return {
        toolName: "BrowserEval",
        params: { script: step.params.script },
      };
  }
}

export function BrowserPanel({ onInvoke, loading, result }: BrowserPanelProps) {
  const [view, setView] = useState<"automation" | "navigate" | "console">(
    "automation",
  );
  ...
  const invokeAndWait = useCallback(
  ...
  const runAllSteps = useCallback(async () => {
    ...
    for (const step of steps) {
      ...
      const res = (await invokeAndWait(toolName, params)) as {
        output?: string;
        type?: string;
      };
      ...
      if (autoScreenshot && step.type !== "screenshot") {
        const ssRes = (await invokeAndWait("BrowserScreenshot", {})) as {
          output?: string;
          type?: string;
        };
        try {
  ...
  const addStep = useCallback((type: StepType) => {
    ...
    setSteps((prev) => [
      ...prev,
      { id, type, params: { ...STEP_DEFAULTS[type] }, status: "pending" },
    ]);
  }, []);
    ...
    <div className="flex h-full flex-col gap-4 p-5 overflow-auto">
        ...
        >
          <Circle
            className={cn(
              ...
              sessionActive ? "text-emerald-400" : "text-muted-foreground/40",
            )}
          />
                      ...
                      <div className="flex-1 flex gap-2 min-w-0">
                        {step.type === "navigate" && (
                          <Input
                            ...
                            onChange={(e) =>
                              updateStepParam(step.id, "url", e.target.value)
                            }
                            placeholder="https://"
                        ...
                        {step.type === "type" && (
                          <>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                updateStepParam(step.id, "text", e.target.value)
                              }
                        ...
                        {step.type === "eval" && (
                          <Input
                            ...
                            onChange={(e) =>
                              updateStepParam(step.id, "script", e.target.value)
                            }
      ...
      {view === "navigate" && (
        <>
              ...
              <div className="divide-y divide-border/30 max-h-48 overflow-auto">
                {tabs.map((t, i) => (
                  ...
                  >
                    ...
                    {t.active && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full border border-cyan-500/30 text-cyan-400 bg-cyan-500/10">
                        Active
                      </span>
                    )}
                  </div>
          ...
          {textOut && !screenshot && !parsedTabs && (
            <OutputCard title="Result" content={textOut} />
          )}
        </>
      )}
          ...
          >
            <div className="space-y-3">
              <Textarea
                ...
                rows={6}
                className="dark resize-none font-mono text-xs bg-zinc-950/80 text-emerald-400 border-zinc-700/50"
              ...
              <div className="flex gap-2">
                ...
                <Button
                  onClick={() => onInvoke("BrowserEval", { script: evalCode })}
                  disabled={loading || !evalCode}
