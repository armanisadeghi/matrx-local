...
| 🚨 **THE CLIENT LAW — this repo CONSUMES canonical systems, it never reimplements them.** An agent UUID, an agent version, an agent's prompt, a resolution ladder, a durable URL, or a rebuilt copy of a server-owned record may not live in this repo. Ask the platform BY NAME (a Mandate key, `file_id`, tool name) and render the answer. **The genuine exception is the OFFLINE local-model path** — it has no server to ask; the test is "does this run with the platform unreachable?" (`CloudChat.tsx` is named *Cloud*Chat and fails it). Written after five incidents traced to this repo, incl. `chat_sync` blind-upserting a rebuilt message row and bricking a live conversation | `/Users/armanisadeghi/code/common-docs/policies/clients-consume-never-reimplement.md` — **read before writing any code that decides which agent, which version, which model, or what a file's URL is** |
| 🚨 **Mandates — which agent runs a step is a DATABASE answer, never a code answer.** *(Renamed from "agent slots" 2026-08-16.)* A **Mandate** is the named job; the DB decides which **Holder** fulfils it. **The Python half already has this: `matrx-ai` runs INSIDE this repo as a CLIENT, and `matrx_ai.configure()` auto-installs `ServerMandateSource` because `app/services/ai/engine.py` supplies both `server_url` and `get_jwt` — so a Mandate resolves over the aidream API and a user's rebind reaches desktop with no deploy.** A client missing a capability is a missing API call to add, never a licence to hardcode. **The Tauri/TS half does NOT have it yet:** `CloudChat.tsx` holds a hardcoded default-chat agent id — the same UUID matrx-frontend already serves from the `chat.default_new_chat` Mandate — so an admin repointing the product's most-used agent silently does not change desktop. Never add an agent UUID here | `/Users/armanisadeghi/code/common-docs/systems/mandates/FEATURE.md` (what a Mandate is) · `/Users/armanisadeghi/code/common-docs/systems/mandates/RUNTIME.md` (THE UNIVERSAL LAW + the exception policy) · worklist rows L1–L4: `/Users/armanisadeghi/code/common-docs/systems/mandates/ROLLOUT.md` |
...
| **Google Workspace on desktop** (`google_workspace` server tool + the `google_email_send` review card). The card IS the Gmail authorization: the engine PARKS the call and never sends — never add a server binding, an engine-side send, or a `user_confirmed` argument | [app/services/delegation/FEATURE.md](app/services/delegation/FEATURE.md) § User-review calls + [desktop/src/lib/google-workspace.ts](desktop/src/lib/google-workspace.ts); campaign log: `/Users/armanisadeghi/code/common-docs/projects/google-oauth-verification/PRODUCTION-ROLLOUT.md` |
