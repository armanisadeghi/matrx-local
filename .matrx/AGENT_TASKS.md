...
- [BUG] Packaged sidecar now syncs and archive-verifies scheduler modules — 2026-08-09 (MXL-D-056, 3c44e3c14)
- [REFACTOR] Two Playwright driver trees collapsed into one — 2026-08-09. `FetchWithBrowser` (`app/tools/tools/network.py`) called `async_playwright()` on every invocation, so the engine could hold TWO headless Chromium trees (~150-250 MB each) on the user's laptop at once — and only the scraper lane's was tracked (`ScraperEngine.driver_pid`) and reaped by `terminate_playwright_tree` on run.py's force-exit path; the tool's tree was invisible to it, i.e. the untracked-orphan class behind "ended unexpectedly" (Hard Rule 0). Kept BOTH capabilities rather than deleting one to win the refactor: the tool's semantics (caller `wait_for` selector, `networkidle` settle, fixed desktop UA + 1920x1080, raw HTML / `body` inner text, no parsing) are genuinely different from the scrape lane's parsed `ScrapeResult`, and the desktop Scraping page exposes them as sibling methods. Fix is a lending API — `ScraperEngine.borrow_browser()` — that hands out the ONE pool's browser; the borrower owns every context it opens and never closes the shared browser. The user's "Headless Mode" off setting is preserved by launching a transient visible browser off the SAME driver, so `driver_pid` still covers it. Degraded case (pool `None`, engine still READY) is now a `capability_install` ActionNeeded routing to Settings → Capabilities instead of a raw ImportError string; a busy pool says "the browser was busy for Ns" instead of a bare `TimeoutError:`. Public contract unchanged (name, args, output text, metadata keys) — verified live over `/tools/invoke`, which is exactly what `use-scrape.ts`'s "local-browser" branch calls. Verified on a real `dev.sh --fresh` engine: one driver child before and after fetches, `wait_for` + full-HTML + concurrent `Scrape`/`FetchWithBrowser` all green, SIGTERM left zero orphaned `chrome-headless-shell`/`run-driver`; degraded state reproduced and confirmed. Tripwire `tests/unit/test_single_browser_pool.py` (11 tests) AST-parses network.py and fails on any `playwright` import or `launch()` call. 780 unit+characterization tests green. Filed while here: MXL-D-076 (the interactive `local_browser` suite's driver is still untracked), MXL-D-077 (`dev.sh --fresh` claims "browsers ready ✓" when the headless-shell binary is missing).
- [FEATURE] Missing Playwright browser is now a visible, one-click-fixable STATE — 2026-08-09. `ScraperEngine.start()` logged a WARNING, set `browser_pool = None` and reported a plain READY, so browser-rendered scraping vanished silently: the Scraping page still offered a "Browser" method that could only fail, and nothing told the user why or how to fix it. Real users DO hit this — browsers are deliberately not bundled in the sidecar (Chrome's nested framework breaks macOS codesign), so `_ensure_playwright_browsers()` downloads them in the BACKGROUND at first boot while the scraper starts at Phase 3 seconds later; a first boot always begins with no pool, and a failed download (offline, captive portal, proxy) leaves it missing forever. New `app/services/scraper/browser_runtime.py` owns the probe, the launch-failure reason and the plain-language `ActionNeeded` (`install_browser_engine`, kind `capability_install`); Phase 3 now annotates the `scraper` service record (`browser_available/browser_code/browser_reason/browsers_path`) and marks it **degraded**, not READY, when rendering is gone — startup and HTTP scrapes are untouched. `GET /browser-runtime/status` + `POST /browser-runtime/install` (SSE) expose it; the install reuses `setup_routes._install_playwright_browsers` (now parameterised by browser name) to fetch only `chromium-headless-shell` (~90 MB download / 190 MB on disk, vs 520 MB for the full first-run chromium) into the world's own `MATRX_HOME_DIR/playwright-browsers`, then calls the new `ScraperEngine.ensure_browser_pool()` so rendering comes up in the RUNNING engine — no app restart. UI: `BrowserRuntimeProvider` (singleton, registers the ActionNeeded handler), `BrowserRuntimeNotice` on both Scraping tabs, and `MethodSelector` disables/badges the Browser pill instead of letting the user pick a doomed method. Verified: real install into an isolated temp home (unavailable → install → available → pool starts → HTTP scrape still works), live `~/.matrx` and dev `~/.matrx-dev` paths resolve to their own worlds with no prompt when populated, empty-path engine still starts READY with working HTTP scrapes, 5 python + 7 vitest regression tests, `tsc -b` clean, 304 unit tests green, `./scripts/smoke.sh` CLEAN.
- [BUG] Boot logged two React errors on every launch (MXL-D-020, MXL-D-021) — 2026-08-09. Surfaced the first time the Playwright E2E harness was provisioned on Arman's primary Mac: `pnpm test:boot` came back 2 passed / 2 failed, and both failures were real app defects rather than test setup. (a) **setState-in-render**: `_push` in `src/hooks/use-unified-log.ts` appended to the log buffer AND synchronously dispatched to every mounted subscriber, so a log emitted during a React render setState'd `LogPanel` mid-render of `AppInner` — a tearing/stale-state hazard, not just console noise. Logging can happen from anywhere including render, so the fix is at the one chokepoint: the buffer append stays synchronous (`getClientLogBuffer()` never lags) while notification batches into a `queueMicrotask` flush that runs after React commits and preserves emit order. (b) **`<button>` inside `<button>`**: `PopoverTrigger` renders its own button by default, so `src/components/chat/PlusMenu.tsx` nested one inside another — invalid HTML that breaks keyboard/AT activation of the inner control; fixed with `asChild`. A repo-wide sweep for the same shape found one more (`src/pages/Voice.tsx` AI-Polish preset chevron), fixed identically. Verified: boot 4/4, full e2e 8 passed / 1 pre-existing skip, `tsc -b` clean, 297 unit tests green.
- [BUG] MXL-D-075 — dependency floors resolved a matrx-utils/matrx-connect pair that ImportErrors — 2026-08-09. `matrx-connect` has imported `matrx_utils.AsyncSingleFlight` in `middleware/auth.py` since 0.1.8, but matrx-utils first exported it in **2.0.1** (and `LEGACY_VISIBILITY_MAP`/`VisibilityLiteral` in **2.0.7**); nothing was removed upstream — symbols were ADDED and no consumer floor moved. Our `matrx-utils>=2.0.0` + `matrx-connect>=0.1.10` therefore resolved matrx-utils 2.0.0 + connect 0.1.10 into `.venv`, and `import matrx_connect.middleware` raised `ImportError: cannot import name 'AsyncSingleFlight'`. Surfaced as the PyInstaller "Failed to collect submodules for `matrx_scraper.server`" warning while collecting matrx_scraper into the frozen sidecar (5189719b1). Latent here only because `matrx_connect/__init__.py` does not eagerly import `middleware` and nothing imported that submodule — the first auth-middleware import would have killed the AI streaming surface (`app/api/ai_routes.py`, `conversation_handler.py`, `remote_tool_bridge.py` are all matrx_connect consumers). Root cause fixed upstream in aidream (matrx-connect **0.1.25** floors `matrx-utils>=2.0.7`, published to PyPI; same defect fixed in matrx-files/matrx-ai) with a `test_server_import.py` regression guard in matrx-scraper; floors raised here to `matrx-utils>=2.0.7` / `matrx-connect>=0.1.25` and `uv sync --all-extras` run. Verified per-module: `matrx_connect.middleware` FAIL→OK, `matrx_scraper.server` now imports, full suite green (1182 passed).
