...
def claude_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    import app.services.coding_sessions.claude_overview as claude_module
    import app.services.coding_sessions.cloud_state as cloud_module
    import app.services.coding_sessions.overview as overview_module
    ...
    claude_module._reset_index_state_for_tests(store)
    asyncio.run(claude_module.warm_index_cache(sessions_root))
    yield {"indexed": indexed, "cli_only": cli_only}
    claude_module._reset_index_state_for_tests(None)
...
def _run_overview(monkeypatch: pytest.MonkeyPatch) -> dict:
    ...
    async def _no_cloud(_provider="claude_code", *, force=False):
...
def _only_claude_code() -> None:
    """These guards are about the Claude Code half of the one list.

    Registering only its adapter keeps them from reading Arman's real ~/.codex
    and Cursor stores, which would make them slow and machine-dependent.
    """
...
def test_delivery_ledger_failure_is_unknown_not_zero(
    ...
    async def _cloud(_provider="claude_code", *, force=False):
        return {
            claude_tree["indexed"]: {
    ...
    async def _unavailable_queue(_provider="claude_code"):
        return None, cloud_module._delivery_ledger_meta(checked=False)

    async def _known_totals():
        return (9, 4), cloud_module._delivery_ledger_meta(checked=True)
...
def test_delivery_total_failure_invalidates_row_delivery_counts(
    ...
    async def _queue(_provider="claude_code"):
        ...
        }, cloud_module._delivery_ledger_meta(checked=True)
...
def test_delivery_ledger_error_log_is_sanitized(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cloud_module, "get_db", lambda: _FailingDb())
    monkeypatch.setattr(cloud_module.logger, "error", _record_error)
...
def test_delivery_ledger_reports_database_initialization_failure(
    ...
    monkeypatch.setattr(cloud_module, "get_db", _unavailable_db)
    monkeypatch.setattr(cloud_module.logger, "error", _record_error)
...
def test_the_screen_never_walks_the_index_tree(
    ...
    monkeypatch.setattr(claude_module, "plan_refresh", _must_not_walk)
    monkeypatch.setattr(claude_module, "read_record_rows", _must_not_read)
    ...
    monkeypatch.setattr(
        claude_module.claude_index_helper,
        "helper_command",
...
def test_a_cold_index_says_so_instead_of_pretending_to_be_empty(
    ...
    claude_module._reset_index_state_for_tests(
...
def test_a_cli_only_session_opens_a_diagnosis_instead_of_a_404(
    ...
    monkeypatch.setattr(overview_module, "cloud_inventory", _no_cloud)
    monkeypatch.setattr(claude_module, "_session_envelopes", _none)
    monkeypatch.setattr(claude_module, "_capture_facts", _empty_capture)
    monkeypatch.setattr(claude_module, "_label_facts", _empty_labels)

    async def _no_acknowledgements(*_a, **_k):
        return {}, {"checked": True, "reason": None, "detail": None}

    monkeypatch.setattr(claude_module, "_delivered_by_this_mac", _no_acknowledgements)
...
def test_diagnosis_marks_delivery_evidence_unavailable_instead_of_empty(
    ...
    monkeypatch.setattr(overview_module, "cloud_inventory", _cloud)
    # Claude Code's diagnosis asks the SAME shared inventory function through
    # its own module, so the stand-in has to be installed in both places.
    monkeypatch.setattr(claude_module, "cloud_inventory", _cloud)
    monkeypatch.setattr(claude_module, "_session_envelopes", _unavailable_envelopes)
    monkeypatch.setattr(claude_module, "_delivered_by_this_mac", _unavailable_acknowledgements)
    monkeypatch.setattr(claude_module, "_capture_facts", _empty)
    monkeypatch.setattr(claude_module, "_label_facts", _empty_labels)
...
def test_malformed_ledger_rows_become_unavailable_not_server_errors(
    ...
    monkeypatch.setattr(cloud_module, "get_db", lambda: _MalformedDb())
    rows, meta = asyncio.run(overview_module._queue_by_session())
    assert rows is None
    assert meta["checked"] is False
...
def test_malformed_envelope_rows_become_unavailable_not_server_errors(
    ...
    monkeypatch.setattr(cloud_module, "get_db", _MalformedEnvelopeDb)
    envelopes, meta = asyncio.run(claude_module._session_envelopes("session"))
