    ...
    try {
      if (action === "retry") await engine.retryCodingSessionDeliveryEnvelope(receiptId);
        ...
        const confirmed = await confirm({
          title: `Discard envelope #${impact.receipt_id}?`,
          description: `${impact.warning} Envelope #${impact.receipt_id} contains ${impact.item_count} event${impact.item_count === 1 ? "" : "s"} (${formatFileSize(impact.payload_bytes)}) from ${impact.provider}, and discarding removes them from this envelope for good. The provider's original session is unchanged.`,
          confirmLabel: "Discard envelope",
          variant: "destructive",
        });
        if (!confirmed) return;
            ...
            <tbody className="divide-y">
              {page?.items.map((item) => (
                <tr key={item.receipt_id}>
                  ...
                  <td className="px-3 py-3 align-top"><div>{formatCount(item.item_count)} event{item.item_count === 1 ? "" : "s"} · {formatFileSize(item.payload_bytes)}</div><div className="font-mono text-xs text-muted-foreground">{item.session_ref ?? "No session reference reported"}</div></td>
