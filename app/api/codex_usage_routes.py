...
def _parse(value: str, name: str) -> dt.datetime:
    try:
        ...
        raise HTTPException(422, detail=f"{name} must be an ISO-8601 timestamp") from exc
...
async def get_codex_usage(
    ...
    try:
        snapshot = await snapshot_service.read(_parse(start, "start"), _parse(end, "end"), refresh)
