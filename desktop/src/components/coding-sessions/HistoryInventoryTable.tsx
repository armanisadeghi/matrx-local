...
export function HistoryInventoryTable({
  ...
  return <div className="space-y-3">
    ...
    <div className="relative overflow-x-auto rounded-md border">
      ...
      <tbody className="divide-y">{pageData.items.map((row) => { const key = keyOf(row); const checked = selected.has(key); return <tr key={key} className={checked ? "bg-blue-500/5" : "hover:bg-muted/30"}><td className="px-3 py-3 align-top"><Checkbox checked={checked} disabled={disabled || !row.import_available || !row.present || (!checked && selected.size >= review.limits.selected_sessions)} onCheckedChange={() => { const next = new Set(selected); checked ? next.delete(key) : next.add(key); onSelectedChange(next); }} aria-label={`Select ${row.title}`} /></td><td className="max-w-80 px-3 py-3 align-top"><div className="truncate font-medium">{row.title}</div><div className="font-mono text-[11px] text-muted-foreground">{row.session_id}</div></td><td className="px-3 py-3 align-top"><div>{row.project_name}</div><div className="text-xs text-muted-foreground">{[row.git_branch, row.worktree_name].filter(Boolean).join(" · ") || "No branch metadata"}</div></td><td className="px-3 py-3 align-top">{changeLabel(row.change_type)}</td><td className="whitespace-nowrap px-3 py-3 align-top">{new Date(row.last_modified_ns / 1_000_000).toLocaleString()}</td><td className="whitespace-nowrap px-3 py-3 align-top">{formatBytes(row.bytes)}<div className="text-xs text-muted-foreground">{row.file_count} file{row.file_count === 1 ? "" : "s"}</div></td><td className="px-3 py-3 align-top">{row.import_available ? <Badge variant="outline">Ready</Badge> : <Badge variant="destructive">Blocked</Badge>} {!row.import_available && <div className="mt-1 max-w-52 text-xs text-muted-foreground">{row.import_blocked_reason ?? "No reason reported"}</div>}</td><td className="px-3 py-3 align-top"><Button variant="ghost" size="sm" onClick={() => void navigator.clipboard.writeText(`claude --resume ${row.session_id}`)}><Copy className="mr-1 h-3.5 w-3.5" />Resume</Button></td></tr>; })}{pageData.items.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-muted-foreground">No reviewed sessions match these filters.</td></tr>}</tbody></table>
    </div>
