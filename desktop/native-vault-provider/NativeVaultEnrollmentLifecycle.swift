...
struct NativeVaultEnrollmentOperation: Equatable {
    let id: UUID
    let verifier: String
    let state: String
...
enum NativeVaultEnrollmentLifecycle {
    static func begin(verifier: String, state: String, generation: String, active: NativeVaultEnrollmentOperation?) -> NativeVaultEnrollmentOperation? {
        ...
        return NativeVaultEnrollmentOperation(id: UUID(), verifier: verifier, state: state, generation: generation)
    }

    static func callbackCode(_ url: URL, for operation: NativeVaultEnrollmentOperation, callback: URL) throws -> String {
        ...
        guard items.count == 2, Set(items.map(\.name)).count == 2,
              ...
              lifecycleConstantTimeEqual(returnedState, operation.state),
