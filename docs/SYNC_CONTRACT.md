...
| **Chat system (conversations, messages, user_requests, tool_calls, media, artifacts, …)** | The CANONICAL LOCAL MIRROR: per-schema SQLite file `~/.matrx/mirror/chat.db` ATTACHed as `chat`, tables generated from the live cloud schema (`schema_mirror/snapshot.json` → `scripts/generate_mirror_schema.py` → `mirror_schema.py`). Local ids ARE the canonical cloud ids. Written by `SQLiteConversationStore` + `ConversationsRepo`/`MessagesRepo` (compat shapes preserved) | **Bidirectional, automatic** (2026-07-13; guarded writes 2026-07-18; explicit-org boundary 2026-08-23): every local-origin write enqueues the `sync_queue` outbox; `app/services/chat_sync/engine.py` pulls incremental keyset pages on `(updated_at\|created_at, pk)`, then drains parents before children. New rows use batched insert-ignore; existing versioned rows use conditional PATCH on `(id, version)`. The payload is projected through an explicit per-table desktop-authoring allowlist; newly-added cloud columns are pull-only by default. Every conversation push must already carry the request's valid `organization_id`; every `user_request`, `message`, `request`, `tool_call`, `media`, and `artifact` payload copies the authoritatively loaded conversation org before HTTP. A missing parent/org dead-letters immediately and never reaches PostgREST. Non-UUID ids/other-user rows are likewise dead-lettered; permanent remote payload errors dead-letter after 5 attempts | Pulls retain timestamp LWW only when no local write is pending. If cloud and local both changed, neither is overwritten: the outbox row becomes `action='conflict'` and stores both projected local and complete remote snapshots. A lost insert response is recognized idempotently when all desktop-authored values already match. A mirrored table with no `version` column is insert-only from desktop and a differing existing row conflicts (`chat.media` was the original case; it gained `version` in the canonical base-contract pass). Tenant identity is published explicitly as `organization_id`; actor identity is not — the cloud stamps `created_by`, and the legacy `user_id` is gone. Field-clears remain a known limit because the codec omits `NULL` values | Soft-delete both sides via `deleted_at`; `ConversationsRepo.delete` tombstones (repo reads filter live rows); nothing in the pull path ever hard-deletes | `sync_meta` rows keyed `chat.<table>`; keyset cursor JSON in `last_hash`; conflicts and org-provenance failures are durable `sync_queue` rows exposed in mirror status | Full local write; outbox drains on reconnect (V10 migration also seeded the outbox with all pre-cutover local history) |
...
| **Models / agents / tools cache** | SQLite `ai_models`, `agents`, `agent_execution_details`, `tools` | **Pull-only cache.** `agents` mirrors the Supabase RPC `public.agx_get_list_full()` — the ONE platform agent catalog, identical structure/format/order (ruling D4; `app/services/agent_catalog/FEATURE.md`). `ai_models` from the AIDream API; `agent_execution_details` is a LAZY fetch-through cache of `public.agx_get_execution_full(p_agent_id)` (the same per-agent read Cloud Chat makes), filled on the first `/agents/catalog/{id}/execution` request and refetched after a 600s TTL — never a startup stampede of one RPC per agent, and never membership; tools from the local catalog | Cloud wins unconditionally — `delete_missing` prunes rows absent from the feed. This is doctrine-compliant: it is a cache of cloud-authoritative catalog data, not user work. Empty-feed guard keeps the cache instead of wiping it | N/A — pruning IS the delete mechanism | `sync_meta` per-entity `last_synced_at` + content hash | Read-only offline: stale cache served, sync skipped with a loud log, `sync_meta.status='skipped'/'offline'` recorded |
...
| **Coding-session events** | SQLite `coding_session_bridge_outbox` → aidream `POST /api/coding-sessions/bridge` → owner-only `chat.coding_session` / `chat.coding_session_entry` | Push-only, strictly ordered **per provider/project/session lane**, exact adapter envelope v1. Direct-loopback command-hook ingress uses an isolated `synchronous=FULL` transaction on the same DB before returning HTTP 202; engine-owned task uploads with the persisted active user's JWT | Provider/server idempotency: a real stable event ID deduplicates an exact pending envelope and any field mutation conflicts; no-ID events keep honest best-effort semantics. Unknown remote outcome replays the same checksum-verified bytes after restart | Only 2xx plus a matching BridgeResponse v1 receipt removes the outbox row; enqueue and acknowledgement summaries are recorded separately without payload/session/path data; raw cloud ledger retention/deletion is owned by aidream | The oldest eligible lane head is attempted first. A deferred/failing row blocks only its own logical session; other providers and sessions continue. A permanently rejected row is preserved in quarantine and explicitly reported as not delivered | Full: hook success/import acceptance means fsynced SQLite only (“queued”), never cloud sync. Offline/auth/server/malformed-ack failure preserves the locally accepted event; only validated receipt means “acknowledged” |
| **Downloads** | SQLite `downloads` | Local-only (device-specific artifacts) | N/A | Status `cancelled` | N/A | Full |
| **User files (old matrx-files mirror) — RETIRED 2026-09-24** | — | Removed: it copied the whole cloud Files tree into `~/Documents/Matrx/Files` as zero-byte placeholders. Replaced by the Rust folder sync (person-picked folders only, nothing by default). One-time placeholder cleanup: `app/services/file_sync/FEATURE.md` | — | — | — | — |
...
4. **`app/services/chat_sync/`** — the chat-mirror subsystem.
   ...
   conversation pushes preserve the request's `organization_id`; all six
   child payload families copy that authoritative organization before HTTP,
   while `created_by` / `updated_by` / `version` remain cloud-owned. The outbound
...
- 2026-08-23 — chat mirror pushes now require an explicit conversation org and
  copy it into all six child payload families before HTTP; missing provenance
  is loudly dead-lettered instead of delegated to database inheritance.
