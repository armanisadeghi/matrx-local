...
**One defeat, found by a fourth seat and fixed here (I1).** The first version of this window derived
the denominator as `synced.len() + recent_deletions` at plan time and its comment claimed that was
"what the mapping held when the window opened". It is not: files **added** during the window inflate
it. Their input — 40 synced, 19 deleted, **200 added**, 19 more deleted — read as 15.8 % instead of
95 % and propagated 38 of the original 40 with no suspension, which is the same sentence H1 was
written about, one input further along. The count is now recorded in the journal by the deletion
that opens the window, from the item count taken **before** that deletion's row is removed, and the
percentage divides by it. A mapping receiving a download, an import or a restore while a local
`rm -rf` walks the tree is exactly that shape. The same seat also measured that a **backwards
clock** hid a deletion from the window (1 of 2 counted); a recorded stamp is now never earlier than
the last one that mapping recorded, so the window is monotonic by construction.
