  ...
  return (
    <div className="flex h-full flex-col gap-4 p-5 overflow-auto">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              autoRefresh ? "border-violet-500/50 text-violet-700 dark:text-violet-400 bg-violet-500/10" : "border-border text-muted-foreground"
            )}
          >
      ...
      {view === "overview" && (
        <>
          {/* Gauge rings */}
          <ToolSection title="System Resources" icon={Activity} iconColor="text-violet-700 dark:text-violet-400" noPadding>
          ...
          {processes.length > 0 && (
            <ToolSection title="Top Processes" icon={Zap} iconColor="text-violet-700 dark:text-violet-400"
              ...
              <div className="flex justify-end gap-4 px-4 py-1.5 text-[9px] border-t border-border/30">
                <span className="text-violet-700 dark:text-violet-400 uppercase tracking-wider">CPU</span>
                <span className="text-blue-700 dark:text-blue-400 uppercase tracking-wider">MEM</span>
              </div>
            </ToolSection>
          )}
        </>
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-background/90 backdrop-blur-sm">
                  <tr className="border-b">
                    ...
                    <th className="text-right px-3 py-2 text-[10px] font-semibold text-violet-700 dark:text-violet-400 uppercase tracking-wider w-16">CPU%</th>
                    <th className="text-right px-3 py-2 text-[10px] font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wider w-16">MEM%</th>
                ...
                <tbody>
                  {filteredProcesses.map((p, i) => (
                    <tr key={p.pid ?? i} className="border-b border-border/20 hover:bg-muted/20 transition-colors group">
                      ...
                      <td className="px-3 py-1.5 text-right tabular-nums text-violet-700 dark:text-violet-400">{(p.cpu_percent ?? 0).toFixed(1)}</td>
                      <td className="px-3 py-1.5 text-right tabular-nums text-blue-700 dark:text-blue-400">{(p.memory_percent ?? 0).toFixed(1)}</td>
                      <td className="px-3 py-1.5 text-center">
                        <span className={cn(
                          "text-[9px] px-1.5 py-0.5 rounded-full",
                          p.status === "running" ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "bg-muted text-muted-foreground"
                        )}>
                          {p.status ?? "—"}
                        </span>
                      </td>
