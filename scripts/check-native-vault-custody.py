...
def census(sources: dict[str, str]) -> list[str]:
    ...
    for path, text in sources.items():
        for symbol in set(re.findall(r'\bSec(?:Item|AccessControl|Random|Task)[A-Za-z0-9_]+\b', text)) if path.endswith((".swift", ".rs")) else set():
        ...
        for symbol in re.findall(r'@_cdecl\("(matrx_vault_[^"]+)"\)', text):
    ...
    for base in ('desktop/src', 'desktop/src-tauri/src', 'desktop/native-vault-provider', 'app'):
        for path in (root / base).rglob('*'):
            ...
            if any(part in EXCLUDED for part in relative.parts) or not path.is_file():
                continue
            if path.suffix not in {'.rs', '.swift', '.ts', '.tsx', '.py'} or '.test.' in path.name or '.spec.' in path.name:
                continue
