...
- 2026-09-18 — **The pin is the app's own starred list, per account; `isStarred` is a diagnostic.** The sidebar draws pins from IndexedDB `keyval-store`/`keyval`/`store:pin-state:dframe-starred-code` (one list per account; rebuilt on every switch — a partial value then the full one — so a list is attributed only when its `updatedAt` postdates the app's own switch marker, and an empty latest value is UNKNOWN). Measured that day: 206 unarchived records flagged `isStarred: true`, ~48 shown pinned, 56 in the list, 285 favourites on AI Matrx. `scripts/claude_code_pins_extract.py` publishes `app_starred`; the session-sync agent keeps each account's last list in `~/.claude/claude-code-pin-observations.json` (a >30%/≥10 shrink is not saved) and computes ONE verdict there, `master` = `{pinned, unpinned, coverage}`: pinned = in any account's list; unpinned ONLY when every account in `claude-code-accounts.json` was observed within 14 days (otherwise empty — partial coverage never unpins); archived and transcript-less conversations excluded. The ledger and `LivePins` (both readers; `CLAUDE_PIN_OBSERVATIONS` overrides the path) apply only `master`; anything else keeps its ledger state. Fix round 1 closed review R-P1 (one observed account had produced ~1,900 explicit unpins). Guards: `tests/unit/test_claude_pin_truth.py`, `tests/unit/test_claude_pins_extractor.py`.
