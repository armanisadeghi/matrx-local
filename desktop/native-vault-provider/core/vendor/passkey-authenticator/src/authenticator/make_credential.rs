    ...
    U: UserValidationMethod<PasskeyItem = <S as CredentialStore>::PasskeyItem> + Sync,
{
    ...
    pub async fn make_credential(&mut self, input: Request) -> Result<Response, StatusCode> {
        self.make_credential_with_backup_flags(input, BackupFlags::DEVICE_BOUND)
            .await
    }

    /// Create a credential with independently validated CTAP backup flags.
    pub async fn make_credential_with_backup_flags(
        &mut self,
        input: Request,
