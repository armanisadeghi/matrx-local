"""Zero-authorship verification of CS-33. Written by a verifier, not the builder.

Deliberately NOT reusing the builder's fixtures or helpers: every tree here is
built from scratch so a fixture that encodes the builder's assumption cannot
launder it.
"""
...
from app.services.coding_sessions import claude_index_store as store_mod
...
from tests.unit.test_claude_session_labels import _FakeClient, env  # noqa: E402,F401
from app.services.coding_sessions.title_sync import (  # noqa: E402
...
async def test_a_blocked_pass_is_not_a_writability_verdict_either(env) -> None:
    ...
    await db.execute(
        """INSERT INTO coding_session_metadata_sync_operations (
               ...
               compared_sessions, index_files, index_writable, error_message
           ) VALUES ('op-blocked', 'apply', 'failed', '2026-09-18T15:45:40Z',
                     '2026-09-18T15:45:40Z', 0, 0, 0, 'no_active_user_jwt')"""
    )
    await db.commit()
