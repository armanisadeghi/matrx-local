...
| Ticks record | none | `publisher.ticks` populated (sent/failed/eligible/blocked) |
...
aidream (`/api/coding-sessions/bridge`), not here. At 149/min the 191k backlog drains in ~21 h;
raising `coding_session_delivery_concurrency` to 16 is the first thing to try on the installed
app once this is released.

Also found and repaired the same afternoon, on the LIVE machine: `~/.matrx/mirror/chat.db` had
corrupt indexes (`PRAGMA integrity_check`: rows missing from `idx_tool_trace_created_by`,
`idx_request_snapshot_created_by`, and more) — the source of every "database disk image is
malformed" crash in chat_sync since 09:05. Backed up with VACUUM INTO, then `REINDEX`,
`integrity_check` → ok, chat_sync pulling again. Root cause is the SIGKILL orphan sweep the Sync
session fixed in c20c9ccf8; a startup integrity check + auto-REINDEX for mirror DBs is the class
fix still owed.
