...
"$SWIFTC" -parse-as-library -sdk "$SDK" -target "$ARCH-apple-macosx15.0" -I "$BRIDGE" -Xcc "-fmodule-map-file=$BRIDGE/native_vault_coreFFI.modulemap" -L "$ROOT/native-vault-provider/core/target/$RUST_ARCH-apple-darwin/release" -lnative_vault_core "$BRIDGE/native_vault_core.swift" "$ROOT/native-vault-provider/NativeVaultCodec.swift" "$ROOT/native-vault-provider/NativeVaultEnrollmentLifecycle.swift" "$ROOT/native-vault-provider/NativeVaultState.swift" "$ROOT/native-vault-provider/NativeVaultPrivateSession.swift" "$ROOT/native-vault-provider/NativeVaultPassword.swift" "$ROOT/native-vault-provider/NativeVaultPasskeyCodec.swift" "$ROOT/native-vault-provider/NativeVaultPasskey.swift" "$ROOT/native-vault-provider/NativeVaultIdentity.swift" "$ROOT/native-vault-provider/CredentialProviderViewController.swift" "$ROOT/native-vault-provider/tests/NativeVaultPasskeyControllerCorpus.swift" -o "$WORK/corpus"
"$WORK/corpus"
