...
function makeResult(meta: unknown, overrides: Partial<ScrapeResultData> = {}): ScrapeResultData {
  ...
  return {
    ...
    success: true,
    status_code: Number(record["status_code"] ?? 200),
    content: "Some extracted page text.",
    title: String(record["title"] ?? ""),
    content_type: String(record["content_type"] ?? "html"),
    response_url: String(record["response_url"] ?? record["url"] ?? ""),
    error: null,
    elapsed_ms: 1200,
    extraction: parseScrapeExtraction(meta),
    ...overrides,
  };
}
    ...
    const html = renderToStaticMarkup(
      <ScrapeResultViewer
        result={makeResult(
          ...
          { success: false, error: "bad_status", status_code: 403 },
        )}
      />,
    );
