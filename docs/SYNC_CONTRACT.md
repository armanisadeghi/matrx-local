...
| **Models / agents / tools cache** | SQLite `ai_models`, `prompt_builtins`, `agents`, `tools` | **Pull-only cache.** `agents` mirrors the Supabase RPC `public.agx_get_list_full()` — the ONE platform agent catalog, identical structure/format/order (ruling D4; `app/services/agent_catalog/FEATURE.md`). `ai_models` from the AIDream API; `prompt_builtins` from the AIDream API as a variables/settings detail cache only, never membership; tools from the local catalog | Cloud wins unconditionally — `delete_missing` prunes rows absent from the feed. This is doctrine-compliant: it is a cache of cloud-authoritative catalog data, not user work. Empty-feed guard keeps the cache instead of wiping it | N/A — pruning IS the delete mechanism | `sync_meta` per-entity `last_synced_at` + content hash | Read-only offline: stale cache served, sync skipped with a loud log, `sync_meta.status='skipped'/'offline'` recorded |
